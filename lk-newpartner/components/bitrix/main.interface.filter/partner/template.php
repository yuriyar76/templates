...
?>
<select name="<?=$field["id"].($bMulti? '[]':'')?>"<?=$params?> class="form-control">
<?
...
<?
		...
		case 'date':
?>
				<select name="<?=$field["id"]."_datesel"?>" onchange="bxGrid_<?=$arParams["GRID_ID"]?>.OnDateChange(this)" class="form-control">
<?
...
?>
				...
				<span class="bx-filter-days" style="display:none"><input type="text" name="<?=$field["id"]."_days"?>" value="<?=htmlspecialcharsbx($arResult["FILTER"][$field["id"]."_days"])?>"  class="filter-date-days form-control" size="5" /> <?echo GetMessage("interface_filter_days")?></span>
				<span class="bx-filter-from" style="display:none"><input type="text" name="<?=$field["id"]."_from"?>" value="<?=htmlspecialcharsbx($arResult["FILTER"][$field["id"]."_from"])?>" class="filter-date-interval form-control"<?=$params?> /><?
...
);?></span><span class="bx-filter-hellip" style="display:none">&hellip;</span><span class="bx-filter-to" style="display:none"><input type="text" name="<?=$field["id"]."_to"?>" value="<?=htmlspecialcharsbx($arResult["FILTER"][$field["id"]."_to"])?>" class="filter-date-interval form-control"<?=$params?> /><?
$APPLICATION->IncludeComponent(
	"bitrix:main.calendar",
	"",
	array(
...
<?
		...
		case 'quick':
?>
<input type="text" name="<?=$field["id"]?>" value="<?=htmlspecialcharsbx($value)?>"<?=$params?> class="form-control">
<?
			if(is_array($field["items"])):
?>
<select name="<?=$field["id"]?>_list" class="form-control">
...
<?
		...
		case 'number':
?>
<input type="text" name="<?=$field["id"]?>_from" value="<?=htmlspecialcharsbx($arResult["FILTER"][$field["id"]."_from"])?>"<?=$params?> class="form-control"> ... 
<input type="text" name="<?=$field["id"]?>_to" value="<?=htmlspecialcharsbx($arResult["FILTER"][$field["id"]."_to"])?>"<?=$params?> class="form-control">
<?
			break;
		default:
?>
...
<?endforeach?>
			...
			<div class="bx-filter-buttons">
				<input type="submit" name="filter" value="<?echo GetMessage("interface_grid_find")?>" title="<?echo GetMessage("interface_grid_find_title")?>" class="btn btn-primary">
				<input type="button" name="" value="<?echo GetMessage("interface_grid_flt_cancel")?>" title="<?echo GetMessage("interface_grid_flt_cancel_title")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ClearFilter(this.form)" class="btn btn-default">
